...
public class MealsUtil {
    ...
    public static Map<LocalDate, Integer> getCaloriesSumByDate(List<Meal> meals) {
        return meals.stream()
    ...
    public static List<MealTo> getMealsTo() {
        List<Meal> meals = getMeals();
        return meals.stream()
                .map(meal -> createTo(meal, getCaloriesSumByDate(meals).get(meal.getDate()) > CALORIES_PER_DAY))
                .collect(Collectors.toList());
    }
